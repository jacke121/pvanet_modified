...
def test_net(net, imdb, max_per_image=100, thresh=0.01, vis=False):
    ...
    # this box file is different from all_boxes file, this is unrelevant with cls
    # and just the original boxes coming from RPN net, didn't tuned by regression
    rpn_all_boxes = [ [] for _ in xrange(num_images) ]
    ...
    for i in xrange(num_images):
        ...
        scores, boxes , rpn_boxes = im_detect(net, im, _t, box_proposals)

        rpn_all_boxes[i] = np.array( rpn_boxes ).astype(np.float32)
    ...
    det_file = os.path.join(output_dir, 'pure_rpn_detections.pkl')
    with open(det_file, 'wb') as f:
        cPickle.dump( rpn_all_boxes, f, cPickle.HIGHEST_PROTOCOL )
